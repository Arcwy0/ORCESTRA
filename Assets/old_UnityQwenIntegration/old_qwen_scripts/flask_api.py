...
DEFAULT_ADAPT_CKPT_PATH = "/app/output_qwen_food"
...
@app.route("/question_unity", methods=["POST"])
def question_unity():
    global history
    history = None
    text_input = request.json.get("text", "")
...
@app.route("/search_unity", methods=["POST"])
def search_unity():
    global history
    history = None
    text_input = request.json.get("text", "")
    ...
    image = Image.open(BytesIO(image_data))
    image = image.convert("RGB")
    image_path = "/app/search_unity_input.jpg"
    image.save(image_path)
    ...
    for i in range(1, len(response_parts)):
        ...
        response_parts[i] = (
            f"<box>({denormalized_bbox_coords[0]},{denormalized_bbox_coords[1]}),({denormalized_bbox_coords[2]},{denormalized_bbox_coords[3]})</box>"
            + rest_part
        )
